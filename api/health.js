...
export default async function handler(req, res) {
  ...
  if (!configured) return res.status(503).json({ status: 'BLOCKED', runtime: 'llm-chat-intel', bridge: 'NOT_CONFIGURED', configured: false });
  ...
  try {
    ...
    return res.status(200).json({ status: 'REAL', runtime: 'llm-chat-intel', bridge: 'RESPONDING', corpus: 'gpt_conversations', receipt: 'HEALTH_QUERY_RETURNED', payload_shape: Array.isArray(payload) ? 'array' : typeof payload });
  } catch (e) {
